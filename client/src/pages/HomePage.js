...
const HomePage = () => {
  ...
  const getAllCategory = async () => {
    try {
      const { data } = await axios.get("https://intelli-shop-server.vercel.app/api/v1/category/get-category");
      if (data?.success) {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`https://intelli-shop-server.vercel.app/api/v1/product/product-list/${page}`);
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("https://intelli-shop-server.vercel.app/api/v1/product/product-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post("https://intelli-shop-server.vercel.app/api/v1/product/product-filters", {
        checked,
        radio,
      });
      setProducts(data?.products);
